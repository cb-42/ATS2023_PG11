...
#### 4.1.1.b Biplot  
A biplot can help us understand associations between certain microbial features, specimens, specimen groups, and/or principal components. In a biplot, the specimens are represented as points (scores) whereas the features are represented as vectors (loadings).  
...
See Supplemental (9.1.1) for several strategies for creating stacked bar charts to compare OTU relative abundances.

### 4.1.3 Trees and heat maps  
See Supplemental (9.1.2) for two approaches of constructing heap maps using lung specimens to compare OTU relative abundances. 

## 4.2 Hypothesis testing  
Next we'll explore how to conduct significance testing.  

### 4.2.1 PERMANOVA (adonis2)  
The following is an example of a PERMANOVA implementation using `vegan::adonis2()`.  

```{r adonis}
# PERMANOVA
set.seed(rnd_seed) # setting a seed allows for reproducibility. Both of these lines need to be run together for it to work.
# adonis(otu_lung_hel~otu_lung$vendor, method="euclidean", permutations = 10000)$aov.tab # 9.999e-05 *** # Note that adonis() will soon be deprecated
adonis2(otu_lung_hel~otu_lung$vendor, method="euclidean", permutations = 10000) # 9.999e-05 ***
...
### 5.1.2 Random forest (Classification: mean decrease in accuracy)  
