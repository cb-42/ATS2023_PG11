...
# I. Introduction
Welcome to the R coding portion of the hands-on introduction to studying the lung microbiome. In this tutorial we'll demonstrate how R can be used to perform data manipulation, modeling, visualization, and reporting as part of our analysis workflow. If you have not already done so, please download R, RStudio, and the necessary packages. See this [guide](https://github.com/cb-42/ATS2023_PG11) for more details.  

## 1.1 Why RStudio?
There are many quality of life features that aid in making your time spent coding more efficient and your work reproducible. For example, many keyboard shortcuts exist in RStudio Desktop for common tasks, such as executing code or adding new code chunks. For a full list of shortcuts, navigate to the top of RStudio's interface, select **Tools** then **Keyboard Shortcuts Help**. There are many other useful features, though a full discussion of these is outside the scope of this presentation.  
...
## 1.3 Navigating RStudio Desktop & an R Markdown script  
...
# Any lines without a # symbol comprise actual R code and  will be executed when this code chunk is run.
# To execute an entire code chunk: left click the green arrow in upper right of this section:
# Alternatively, left click on the following line with R code, then use: cmd + enter (mac), ctrl + enter (pc). This will run a single line rather than
# the entire chunk.
paste("Welcome to ATS 2023, PG11: A Crash Course in Microbiome Analysis")
```

## 1.4 Brief notes on R code syntax, best practices  
These following suggested practices will aid you as you begin coding. We'll also demonstrate these throughout the tutorial.  

  - Include comments explaining what your code does. The primary beneficiary will likely be your future self!  
  - To view the help documentation for a function or library, use a `?` and `function_name`, such as `?ggplot`.  
  - Give objects meaningful, formulaic names to signal or recall what is inside the object. This is not always easy when creating dozens of variables.  
  - Utilize the following syntax where appropriate: `package_name::function_name()`  
      - The main reason to do this is to avoid calling the wrong function of the same name from another package, as we'll see with `dplyr::select()`. It also informs users which library a function is from. 
  - Become familiar with and make use of the pipe operator: `%>%`. The pipe passes the output of a function as input to another function. This can drastically improve the efficiency of your time spent coding, and is a natural fit for data wrangling workflows.  We'll see several examples of this in action.  
...
library(mvabund)        # mvabund(), manyglm() # Note: mvabund modeling section is commented out by default due to its time cost
...
## 2.2 Variable setup
It's often beneficial to include a code chunk containing variables that store settings to be used throughout a workflow. Let's first establish a seed for reproducibility.  

Secondly, we'll create a custom color palette for use in certain visualizations. It's beneficial to define and organize variables early in the workflow, as code that relies on the palette won't run if the palette hasn't been defined. Thus it is best to define the color palette before creating any visualizations.  

```{r variable_setup}
rnd_seed = 8947 # seed to be used throughout workflow where functions would otherwise yield pseudo-random output
# for PCA ordinations; choose an 9 value pal. covering maximum number of factor levels (conditions) within variable of interest
...
# Best practices: 
# 1) keep a separate project for each experiment or combination of experiments
...
# For this tutorial, all sources files can be found in the data subfolder.

# I. Read in .shared file (counts of sequences assigned to OTU #s)
# The following code includes basic processing with selection of OTUs > 0.1 % of the population
# First read in the data, treating the first row as column names
otu_raw <- read.table("data/Heterogeneity.shared.txt", row.names = 2, header = TRUE, stringsAsFactors = FALSE) 
...
otu_good <- otu_trim[, which(colSums(otu_trim) > 0)] # keep only OTUs remaining after setting quantities under 0.1 to 0

# Trim _S from end of rownames(otu_good); it may be useful to keep these well identifiers in some analyses
# Also clean up extra underscores and Xs from rownames
rownames(otu_good) <- str_remove(rownames(otu_good), "_S\\d+") %>% # before assigning output to rownames(), it's piped below
  str_replace_all(pattern = "[_X]+", replacement="_") %>%
  str_remove(pattern="_$")


# It's a good idea to inspect data regularly with tools such as dim(), str(), head(), tail(), summary(), etc. This will help catch errors
dim(otu_good) # dim(otu_good)[1] observations (samples) x dim(otu_good)[2] features (OTUs above .1% population threshold)
otu_good[1:20, 1:10] # Examine first 20 rows, 10 columns. Try using other means for glimpsing the data, such as str(otu_good).


# II. Read in cons.taxonomy (for each OTU: contains taxonomic identifiers down to Genus level, if classification was possible via mothur)
# Note that OTUs are in rows here rather than columns
# 2 imports are required due to the combination of 2 different separators in the source file 
...
tax2 <- read.table("data/Heterogeneity.0.03.cons.taxonomy.txt", sep = ";", skip = 1, col.names = c("", "Phylum", "Class", "Order", "Family", "Genus", ""),
    ...
    purrr::map_df(stringr::str_replace, "\\(.*.\\)", "") %>% # this essentially trims '(100)' and such
    purrr::map_df(as.factor) # turns all remaining columns into factor type variables
otu_taxonomy <- data.frame(OTU = rownames(tax1), Size = tax1[, 1], tax2, stringsAsFactors = FALSE) # combine first and second import
...
# otu_good_taxonomy <- as.data.frame(otu_good_taxonomy) # redundant in this case - we've already got a dataframe object

dim(otu_good_taxonomy) # Displays number of rows and columns
otu_good_taxonomy # Examine the taxonomy dataframe. The first 10 OTUs coincide with the glimpse we took of otu_good. 
# Note: we leverage the df_print: paged RMarkdown YAML option to be able to look through the dataframe in HTML output.  
```

## 2.4 Metadata  
In order to group our samples by factors of interest, we'll also need metadata or environmental data. Here we illustrate how that can be done.  
...
otu_df <- data.frame(meta_df, vegan::decostand(otu_good, "total") * 100, stringsAsFactors = FALSE) # standardize OTUs in our otu dataframe


### Optional reading: Examples of metadata creation using regular expressions (regex) in a tidyverse pipeline ###
...
#                                        str_detect(Sample_name, "^[:alnum:]+") ~ str_extract(Sample_name, "^[:alnum:]+")), # for experimental groups, place after other steps as this regex fills in values for controls (may or may not be ideal)
...
Here is a glimpse of the metadata, done by randomly sampling 10 rows. Both lines need to be run at the same time for the seed to be used properly.  

```{r view_metadata_sample}
# The following code uses our seed for reproducibility, then randomly samples and displays 10 rows from the metadata dataframe
set.seed(rnd_seed) # feel free to explore other samples by changing the seed to another integer
dplyr::sample_n(meta_df, 10)
```
...
## 3.1  ddPCR  
We'll show an example using ddPCR, though first we need to load the data.  

```{r abs_abundance_prep}
# The readxl library is installed along with tidyverse, but is not loaded with library(tidyverse). We can however access
# the read_xlsx function without loading readxl as shown below with the readxl::read_xlsx notation.
spec_abs <- readxl::read_xlsx("data/16S_EvaGreen__Heterogeneity1_Data.xlsx", sheet = 2) %>%
  dplyr::select(Sample_name = `Sample Name`, DNA_copies_per_ul = `Copies/ul DNA`) %>% # Rename the two columns of interest, retain only these columns
...
ctrl_abs <- readxl::read_xlsx("data/16S_EvaGreen__Heterogeneity1Ctrls_030618.xlsx", sheet = 2) %>% 
  dplyr::select(Sample_name = `Sample Name`, DNA_copies_per_ul = `Copies/ul Sample`) %>% # Rename the two columns of interest, retain only these columns
...
avg_abs_df <- abs_df %>%
  group_by(Type) %>% # Before computing an average, identify categories for aggregation. In this case, they're contained within the Type variable we created.
...
ggplot(avg_abs_df, aes(x = Type, y = mean_DNA_copies, fill = Type)) + # This brings in the dataframe and assigns columns to various plot elements
  ...
  # geom_dotplot(data = abs_df, aes(y = DNA_copies_per_ul), binaxis = "y", dotsize = .6, stackdir = "center", stackratio = 1.5) + # optional
...
There are several tools and methods we can use to compare the makeup of microbial communities.  

## 4.1 Visualization techniques  
In this subsection we will explore visualization methods including PCA ordinations, biplots, and rank abundance plots (ordered bar charts).  
...
It's important to compare the distribution of bacteria present in specimens and negative controls to gauge possible contamination. Let's construct our first PCA model with a comparison of lung tissue and negative controls in mind.  

```{r prep_pca_con_lung}
# Filter data to retain controls and lung tissue only
con_lung_df <- otu_df %>% # Note: this is one of several possible approaches to filtering to retain the correct rows.
  dplyr::filter((reagent_tissue == "Control") | (tissue_type == "Lung")) %>% # 65 specimens
  droplevels() # Remove factor levels with no remaining observations. This prevents potential headaches in downstream data wrangling and visualization.
...
otu_con_lung_mat <- otu_good[con_lung_df$specimen, ]
otu_con_lung_hel <- decostand(otu_con_lung_mat, "hellinger") # Standardize: use OTU columns only
...
# prepare x and y-axis labels
xlab = paste0("PC1 (",round(otu_con_lung_pca$CA$eig[1]/sum(otu_con_lung_pca$CA$eig)*100,2),"% Explained)")
ylab = paste0("PC2 (",round(otu_con_lung_pca$CA$eig[2]/sum(otu_con_lung_pca$CA$eig)*100,2),"% Explained)")
```

Next, we see the corresponding ordination (a spider plot). Clear separation is evident between negative controls and lung specimens along PC1.  

```{r spider_con_lung_final, fig.height=10, fig.width=10}
# This approach combines base R plotting functionality and vegan's ordispider function, which creates lines connecting points to the centroids.
# Unlike with ggplot(), these lines of code must all be run at once in order to produce the desired result.
plot(otu_con_lung_pca, type = "n", font = 2, font.lab = 2, xlab = xlab, ylab = ylab, main = "PCA of Lung Tissue vs Controls, colored by Type", display = "sites")
...
ordispider(otu_con_lung_pca, con_lung_df$reagent_tissue, label = TRUE, show.groups = c("Control", "Tissue"), col = pcapal[c(1,2)]) # Draw lines from centroids to respective group's points. 
...
Next we'll add the points with the `points()` function. Note that we're setting the point shape and color with the `pch` and `col` parameters, respectively. We also have to run both of these lines at the same time to get the desired result.  
...
We finish the main elements of a spider plot by drawing lines between the points and centroids, done with `ordispider()`. The second argument tells the function which dataframe variable we want to use for the centroids.  
...
`vegan` also supports additional ordinations such as ellipses. Simply adding an `ordiellipse` call with a color parameter (`col`) will give us the following plot. Although other plot customizations could be made, this is where we'll stop for this example.
...
#### 4.1.1.b Biplot  
A biplot can help us understand associations between certain microbial features, specimens, specimen groups, and/or principal components. In a biplot, the specimens are represented as points (scores) whereas the features are represented as vectors (loadings). Key points for interpreting biplots:

  - Use the cosine of an angle between two features to derive the coefficient of correlation between these features.
  - This can also be applied to the angle between a feature and a principal component.
  - Longer feature loadings indicate stronger influence on a given PC, whereas 0 length loadings indicate no influence. 

Note how Otu0001 and Otu0003 appear to be strongly associated with a subset of negative controls. We see indications of other features aligning with other subsets of specimens.  

```{r biplot_con_lung, fig.height=10, fig.width=10, warning=FALSE}
# Consider whether rownames should be shown, if so they may need to be truncated for visibility
# biplot(otu_con_lung_pca, type = "t") # This can result in many warnings if warning=FALSE is not specified above, though it generates the plot it's not recommended

# Address warnings with the following function, (hides warnings from 0-length arrows)
# Alternatively, set warning=FALSE in the code chunk; does not seem to work on macs in older R/package versions 
...
biplot_results <- quiet_biplot(otu_con_lung_pca, type = "t", font = 2, font.lab = 2, xlab = xlab, ylab = ylab,
...
legend("topright", levels(con_lung_df$reagent_tissue), pch = 19, col = pcapal[c(5,2)], title = "Type") # adjust legend color pal to match points
...
In our original experiment, we compared murine specimens from Charles River and Jackson Labs. Let's create a second PCA model to contrast lung specimens across vendors.  

```{r prep_pca_lung}
# Filter data to retain lung tissue only
otu_lung_df <- dplyr::filter(otu_df, tissue_type == "Lung") %>%
  droplevels() # remove levels corresponding to data that is no longer present due to filtering
rownames(otu_lung_df) <- otu_lung_df$specimen

# PCA of lung tissue specimens
# Keep only the rows of interest for unstandardized data.
otu_lung_mat <- otu_good[otu_lung_df$specimen, ]
otu_lung_hel <- decostand(otu_lung_mat, "hellinger") # Standardize use OTU columns only
...
# prepare x and y-axis labels
xlab = paste0("PC1 (",round(otu_lung_pca$CA$eig[1]/sum(otu_lung_pca$CA$eig)*100,2),"% Explained)")
ylab = paste0("PC2 (",round(otu_lung_pca$CA$eig[2]/sum(otu_lung_pca$CA$eig)*100,2),"% Explained)")
```

```{r spider_lung, fig.height=10, fig.width=10}
# This approach combines base R plotting functionality and vegan's ordispider function, which creates the lines connecting points to the centroids.
# Unlike with ggplot(), these lines must all be run at once in order to produce the desired result.
plot(otu_lung_pca, type = "n", font = 2, font.lab = 2, xlab = xlab, ylab = ylab, main = "PCA of Specimen Origin, colored by Vendor", display = "sites")
points(otu_lung_pca, pch = 19, col = pcapal[as.numeric(otu_lung_df$vendor)]) # pch determines the point shape and col determines the colors to be used.
ordispider(otu_lung_pca, otu_lung_df$vendor, label = TRUE,  show.groups = c("Charles_River", "Jackson_Labs"), col = pcapal[c(1,2)]) # Draw lines from centroids to respective group's points.
legend("topright", levels(otu_lung_df$vendor), pch = 19, col = pcapal, title = "Vendor") # Format and add details to legends
```

#### 4.1.1.d Biplot  
The PCA ordination of lung samples by vendor again shows clear separation of specimens by PC1 (though remember that this PC1 differs from the lung vs control PC1). Next, we'll examine another biplot using only lung specimens. Optional: uncomment the other biplot code to examine alternative approaches. Interestingly, we see that Otu0018 and Otu00052 are clearly negatively associated along PC2. This is in contrast to the negative control vs lung tissue biplot where these features had similar angles and lengths along PC1 and PC2.  

```{r biplot_lung, fig.height=10, fig.width=10, warning=FALSE}
# Consider whether rownames should be shown, if so they may need to be truncated for visibility
...
biplot_results <- quiet_biplot(otu_lung_pca, type = "t", font = 2, font.lab = 2, xlab = xlab, ylab = ylab,
                               main = "Biplot: Specimen Origin, colored by Vendor", display = "species", xlim = c(-0.8, 0.62)) # Note, plot zoom for xlim/ylim may need to be adjusted due to changing the display parameter
points(otu_lung_pca, pch = 19, col = pcapal[c(5,2)][as.numeric(otu_lung_df$vendor)]) # color palette adjusted to conflict less with biplot arrows
legend("topright", levels(otu_lung_df$vendor), pch = 19, col = pcapal[c(5,2)], title = "Vendor")

# example for replacing specimen labels with CR or JL
# biplot(prcomp(otu_lung_hel), xlabs=ifelse(str_detect(otu_lung_df$vendor, "Charles_River"), "CR", "JL")) # arrow.len = 0 will also remove warnings, at the cost of losing the arrowhead
```
#### 4.1.1.e Point Scaling and Plotting other Principal Components  
Optional: See Supplemental (9.1.1.a and b) to learn about point scaling and plotting other Principal Components in PCA ordinations.

### 4.1.2 Rank Abundance  
In the following series of examples, we'll use Rank Abundance (RA) plots to compare the community composition in lung specimens against negative controls. Note that the strategies we'll use can be extended to conduct comparisons across any arbitrary groups, assuming metadata exists to identify the observations (rows) belonging to each group. For instance, we might compare four different types of tissue, or drill down into the controls and compare those subtypes.  
...
ra_df <- otu_df %>% # Note that this is one of several possible approaches to filtering to retain the correct rows
  dplyr::filter((reagent_tissue == "Control") | (tissue_type == "Lung")) %>% # 65 specimens
...
g_df <- ra_df %>%
  dplyr::select(str_which(colnames(ra_df), "Otu", negate = TRUE), colnames(ra_df[, otu_order[1:20]])) %>% # Keep non-OTU columns, followed by the 20 most abundant OTUs
...
# dimg(g_df) # dimensions have gone from 65 745 to 1300 18

# Generate mean and sem for each group
con_lung_agg_df <- g_df %>%
...
First we'll create a basic rank abundance plot, then it will be gradually enhanced in the following examples. A plot can be saved to a variable, such as `p` in this case. Wrapping the code in `()` enables the output to be saved to `p` and viewed at the same time.  

```{r plot_ra_basic, fig.height=6, fig.width=12}
(p <- ggplot(con_lung_agg_df, aes(x = OTU, y = Mean_perc, fill = reagent_tissue)) + # Utilize the data and map the variables to plot parameters.
  geom_col() +
...
```{r plot_ra_errorbar, fig.height=6, fig.width=12}
# Add an errorbar
(q <- p + geom_errorbar(aes(ymin = Mean_perc - SEM, ymax = Mean_perc + SEM, width = 0.3))) 
```

```{r plot_ra_themebw, fig.height=6, fig.width=12}
...
Add Genus labels to OTU identifiers to improve feature interpretation.      

```{r plot_ra_otu_lab, fig.height=6, fig.width=12}
# Add Genus levels to the OTU Number
levels(con_lung_agg_df$OTU) <- fct_inorder(paste0(otu_good_taxonomy[levels(con_lung_agg_df$OTU), ]$Genus, " (", levels(con_lung_agg_df$OTU), ")"))
# equivalent to: # cbmbtools::paste_tax(levels(agg_df$OTU))

ggplot(con_lung_agg_df, aes(x = OTU, y = Mean_perc, fill = fct_rev(reagent_tissue))) + 
  geom_col() +
  facet_grid(fct_rev(reagent_tissue) ~ .) + # use fct_rev to place Tissue first, which is what we've sorted OTUs by
...
con_lung_agg_df$reagent_tissue <- fct_recode(con_lung_agg_df$reagent_tissue, Lung = "Tissue") %>%
  fct_relevel("Lung")

ggplot(con_lung_agg_df, aes(x = OTU, y = Mean_perc, fill = reagent_tissue)) + 
...
ggplot(con_lung_agg_df, aes(x = OTU, y = Mean_perc, fill = Phylum)) + 
  geom_col() + # use show.legend = FALSE to remove the legend entirely
  facet_grid(reagent_tissue ~ .) +
  geom_errorbar(aes(ymin = Mean_perc - SEM, ymax = Mean_perc + SEM, width = 0.3)) + 
  theme_bw() +
...
# Add Genus levels to the OTU Number
levels(lung_agg_df$OTU) <- fct_inorder(paste0(otu_good_taxonomy[levels(lung_agg_df$OTU), ]$Genus, " (", levels(lung_agg_df$OTU), ")"))
```

This time we'll create the final RA plot in one pass.  

```{r plot_ra_lung, fig.height=8, fig.width=12}
(ra_vend_plot <- ggplot(lung_agg_df, aes(x = OTU, y = Mean_perc, fill = Phylum)) + 
  geom_col() + # use show.legend = FALSE to remove the legend entirely
  facet_grid(vendor ~ .) +
  geom_errorbar(aes(ymin = Mean_perc - SEM, ymax = Mean_perc + SEM, width = 0.3)) + 
  theme_bw() +
...
Optional: See Supplemental (9.1.2) for several strategies for creating stacked bar charts to compare OTU relative abundances.  

### 4.1.3 Trees and heat maps  
Optional: See Supplemental (9.1.3) for two approaches of constructing heap maps using lung specimens to compare OTU relative abundances.  

## 4.2 Hypothesis testing  
Next we'll explore how to conduct significance testing.  

### 4.2.1 PERMANOVA (adonis2)  
The following is an example of a PERMANOVA implementation using `vegan::adonis2()`, comparing lung specimens by vendor. We find a significant difference between these groups, consistent with prior visual exploration.   

```{r adonis}
set.seed(rnd_seed) # setting a seed allows for reproducibility. Both of these lines need to be run together for it to work as expected.
...
# adonis(otu_lung_hel~otu_lung_df$vendor, method="euclidean", permutations = 10000)$aov.tab # 9.999e-05 *** # Note that adonis() will soon be deprecated
adonis2(otu_lung_hel~otu_lung_df$vendor, method="euclidean", permutations = 10000) # 9.999e-05 ***
```

### 4.2.2 mvabund  
Because this may take some time to finish (about 4.5 minutes on a powerful machine), the following code is commented out to prevent it slowing down the creation (knitting) of our report. To run this section at a future point that's convenient for you, begin by simply highlighting all of the lines in the code chunk and then using **CTRL + Shift + C** (Windows) or **Shift + Command + C** (Mac) to uncomment the code. For code chunks that take a while to execute, whether due to statistical analyses or visualizations, it may be beneficial to set the code chunk `cache` parameter to `TRUE`. This will enable you to recreate your report much more quickly when making small changes elsewhere.  
...
# otu_mva <- mvabund(otu_lung_df[,str_which(colnames(otu_lung_df), "Otu")]) # select only lung specimens and Otu columns
# otu_ven_many <- manyglm(otu_mva ~ otu_lung_df$vendor)
...
### 4.2.3 Constrained Ordination: Redundancy Analysis 
The following is an example using `vegan::rda()` on the `Cage` variable. For examples of CCA (correspondence analysis) and other ordination techniques, check out the [Intro to Vegan](https://cran.r-project.org/web/packages/vegan/vignettes/intro-vegan.pdf).  

```{r redundancy_analysis}
# Run RDA on our hellinger transformed data. We'll need to pass in one or more vectors of metadata corresponding to the constraining variables.
lung_meta <- dplyr::filter(otu_df, specimen %in% rownames(otu_lung_df)) %>%
  dplyr::select(-str_which(colnames(otu_df), "Otu"))
cage_rda <- rda(otu_lung_hel ~ lung_meta$cage) # First create the rda model object
anova(cage_rda) # Analyze the model
```

# V. Relative Abundance of Key Community Members  
Next we'll further explore the influence of key OTUs and Families.  

## 5.1 Identifying enriched taxonomic groups  

### 5.1.1 mvabund  
While we could make use of mvabund results if the prior mvabund code chunk was run, then examine whether there are any significant taxa, for time considerations we'll show only the code to do so.  
...
# otu_ven_sigpvals
# 
# # Example output (unformatted)
# #     OTU                             uni_p
# #     <fctr>                          <dbl>
# # Otu00002	Streptococcus (Otu00002)	0.001
# # Otu00025	unclassified (Otu00025)	0.001
# # Otu00041	Lactobacillus (Otu00041)	0.017
# # Otu00085	unclassified (Otu00085)	0.018
# # Otu00102	unclassified (Otu00102)	0.021
# # Otu00092	unclassified (Otu00092)	0.022
```
...
# mva_sigpvals # default RStudio output
...
### 5.1.2 Random forest  
Though random forests can be used for prediction, we're especially interested in their feature importance scores. One of the strengths of random forests is that they perform well at handling many correlated features (as is true in the microbiome), as well as a mix of feature types.  

#### 5.1.2.a Classification  
...
```{r family_prep}
# Aggregate the OTU data into family. This requires a fair amount of data reshaping and joining operations. Wrapping these steps into a function is ideal to avoid to retracing every step for simply changing a taxonomic level for aggregation (e.g. Phylum vs Family).
fam_df <- otu_df %>%
  ...
  dplyr::inner_join(dplyr::select(otu_good_taxonomy, OTU, Family), by = "OTU") %>% # Determine which columns to join from taxonomy table. Here we're adding the Family column.
  dplyr::group_by(specimen, Family) %>%
  dplyr::summarize(Percentage = sum(Percentage), .groups = "drop") %>% # Note that this step removes any metadata columns which aren't part of the grouping
...
a <- Sys.time()
set.seed(rnd_seed) # seed for reproducibility
# Setting the importance parameter to TRUE allows us to obtain the variable importance info we're interested in.
rf_vendors <- randomForest(vendor ~ ., data = rf_df, importance = TRUE) 
b <- Sys.time()
b-a
...
In order to better gauge the robustness of these findings, let's create 100 random forest models using the same parameters and input (note: this took around 30 seconds on a somewhat powerful machine).

```{r many_rf_cls, cache = TRUE}
a <- Sys.time()
nforest <- 100
set.seed(rnd_seed) # seed for reproducibility
...
fi_df <- purrr::map(rf_list, ~{.[["importance"]][, 3]}) %>% # 3 specified the index of the importance column (classification)
  unlist() %>%
  data.frame(Feature = names(.), MeanDecreaseAccuracy = .) # rename the FI column to MeanDecreaseAccuracy
...
Next we'll create boxplots of the feature importances over the 100 random forests. From this visual exploration we can more confidently conclude Lachnospiraceae were among the top two most important features in this model. Although these features are ordered by mean importance, also note the degree of overlap of importance across features. Any features whose range includes 0 should likely be excluded from a discussion about microbial importance.   

```{r rf_feature_importance_cls, fig.height=10, fig.width=8}
ggplot(fi_df, aes(x = Feature, y = fi_df[, 2])) + 
  geom_boxplot(outlier.alpha = 0.4) + 
  theme_bw() + 
  labs(y = meas, title = paste0("Feature Importance: vendor ~ Family microbiome"), 
       subtitle = paste0("Model: rf_df \nRuns: ", length(rf_list), ", mtry: ", rf_list[[1]]$mtry)) + 
  coord_flip() + # Flip the axes for improved readability
  scale_x_discrete(limits = rev(levels(fi_df$Feature)))
```

#### 5.1.2.b Regression  
Optional: See Supplemental (9.1.3) for Random Forest Regression.  
...
### 6.1.1 Shannon diversity & dominance  
...
# write.table(rarefy_raw,file = "rarefy.txt") # Output the table if desired
```
...
### 6.1.3 Rank abundance plots  
As we've already seen, RA plots can also be used as a means of exploring α-diversity.  

## 6.2 𝛽-diversity: across specimens  
Next we'll examine diversity across specimens.  

First, we revisit the PCA ordination of lung specimens by vendor to recall global dissimilarity between vendors:  
...
bc_df <- sub_df %>%
  ...
  vegan::vegdist(diag = TRUE, upper = TRUE) %>% # method = "bray" by default
  as.matrix() %>%
...
Next, plot within vendor tissue-tissue BC comparisons.  
...
ggplot(bc_tissue_vend_df, aes(x = vendor, y = Mean_BC, fill = vendor)) + 
  ...
  geom_point(size=1, alpha=0.5, position = position_jitter(width = 0.175, seed = rnd_seed)) + # add 'noisy' points for specimens (adjust width to improve visibility on discrete x-axis)
  theme_bw() + # A minimalist theme
...
# VII. Reporting  
Usually it's not enough to generate statistics or create visualizations. We also need to be able to share these results. Fortunately, there are a variety of ways to do this.  
...
write_csv(otu_lung_df, "otu_lung.csv")
```

The following code chunk creates dataframes aggregated to various taxonomic levels and saves them as .csv files in the project directory. Bonus: instead of using the following loop to write these tables to files, they could be added to the global environment instead, for use in further analysis.  
...
for(taxon in c("Phylum", "Class", "Order", "Family")) {
  taxa_prep_df %>% # Contains results of otu_good after decostand
    ...
    # Change the "_df_ex" if another naming scheme is preferred
}
```
...
Note that there will also be a figure-html folder (in this case: **PG11_Lung_Microbiome_Tutorial_files/figure-html/**) containing all of the plots created through the code chunks in this RMarkdown file. These are used in rendering the final report after finishing step 7.4, and these images could be retrieved from this folder after the knitting process is completed. If your report generates many figures, this might be a good way to go for non-reporting needs.  

Additionally, while within `PG11_Lung_Microbiome_Tutorial.Rmd` and clicking 'Show in New Window' above a plot, then right clicking on the subsequent image displayed will bring up an option to save the image.  

Yet another way to save a plot is to first save a plot to an object, for instance `p`, then type `p` followed by return in the console. Finally, navigate to the plot tab in RStudio and click the export drop down to reveal several methods for exporting the visualization that was just created.  
...
```{r saveRDS}
saveRDS(rf_vendors, "rf_vend_model.rds") # save
rf_vendors2 <- readRDS("rf_vend_model.rds") # load; it's saved to a new object to allow comparison (it should contain identical information)
```

## 7.4 Generating a report: Knitting  
If all goes well, we should also be able to create a report from the narrative and code contained within this R Markdown document that we've been working on. Click the **Knit** button (look for the blue ball of yarn with knitting needle) near the top of RStudio interface to generate the default report type, which is set to HTML for this document. An HTML document containing this report will be created in the current working directory and it will also be displayed in a popup window or in the RStudio Viewer pane. We can easily change the type of file that is created by altering the output type in the header at the top of this document.  
...
[ATS 2021 PG11 materials](https://github.com/cb-42/ATS2023_PG11)  
...
### 9.1.1 PCA Ordination  
#### 9.1.1.a Point scaling  

Let's scale the point size with a quantitative variable and change point opacity. Note that it is easier to judge absolute differences in a quantitative variable using bars (for example) rather than point areas, but this can give us a general idea about the the measurements.  
...
plot(otu_lung_pca, type = "n", font = 2, font.lab = 2, xlab = xlab, ylab = ylab, main = "PCA of Specimen Origin, colored by Vendor", display = "sites")
points(otu_lung_pca, pch = 19, col = alpha(pcapal[as.numeric(otu_lung_df$vendor)], 0.6), cex = 2*otu_lung_df$log_IL1a) # Note the use of alpha around the color vector and cex for the size. The size scale is doubled to make differences more apparent.
ordispider(otu_lung_pca, otu_lung_df$vendor, label = TRUE,  show.groups = c("Charles_River", "Jackson_Labs"), col = pcapal[c(1,2)]) # Draw lines from centroids to respective group's points.
legend("topright", levels(otu_lung_df$vendor), pch = 19, col = pcapal, title = "Vendor") # Format and add details to legends
```

#### 9.1.1.b Plotting other Principal Components  

Suppose we want to examine PC1 and PC3 instead of PC1 and PC2. The `choices` parameter allows us to select PCs besides the PC1 and PC2 defaults. We'll also need to update the axis labels accordingly.
...
# revise y-axis label
ylab = paste0("PC2 (",round(otu_con_lung_pca$CA$eig[3]/sum(otu_con_lung_pca$CA$eig)*100,2),"% Explained)")

plot(otu_lung_pca, choices = c(1,3), type = "n", font = 2, font.lab = 2, xlab = xlab, ylab = ylab, main = "PCA of Specimen Origin, colored by Vendor", display = "sites")
points(otu_lung_pca, choices = c(1,3), pch = 19, col = alpha(pcapal[as.numeric(otu_lung_df$vendor)], 0.6), cex = 2*otu_lung_df$log_IL1a) # Note the use of alpha around the color vector and cex for the size. The size scale is doubled to make differences more apparent.
ordispider(otu_lung_pca, choices = c(1,3), otu_lung_df$vendor, label = TRUE,  show.groups = c("Charles_River", "Jackson_Labs"), col = pcapal[c(1,2)]) # Draw lines from centroids to respective group's points.
legend("topright", levels(otu_lung_df$vendor), pch = 19, col = pcapal, title = "Vendor") # Format and add details to legends
```

### 9.1.2 Rank Abundance  

#### 9.1.2.a Stacked Bar examples  
Stacked bars also allow us to view trends across samples, at the cost of potentially making it more difficult to clearly measure differences in OTUs. There are several strategies to consider when creating such a visualization. Sometimes, when there are many OTUs within a taxonomic level, it may be of interest to combine levels or combine groups of related OTUs to avoid hundreds of very small lines on the resulting bars. This improves the overall visual clarity, though there should be a compelling reason for performing the grouping operation. Additionally, just as in the Rank Abundance visualizations we've already seen, how categories (OTUs or otherwise) are ordered is an important component in clearly presenting one's results.  
...
What would happen if we used the OTU column to fill corresponding portions of the bars? Note that because we've excluded all but the top 20 OTUs, the `position` parameter needs to changed from what was used in the Phyla visualization. Without changing this parameter, the implication is that these 20 OTUs comprise 100% of the data, which is not true.  
...
Suppose we didn't care for the white space, which represents the abundance of OTUs outside the top 20. All of the OTU data could be shown instead. In this case, careful consideration needs to be given for how to effectively represent the results. It's clear that showing hundreds or thousands of OTUs in a single bar is not practical, and may also distract from other signals. Additionally, including hundreds of OTUs in a legend creates a plot that becomes difficult to examine (expanding the `fig.height` and `fig.width` parameters in the respective code chunk label can help). Most likely, many of the OTUs will need to be collapsed into an 'Other' category, or into related groups. The next code chunk contains an example of this, along with additional visual aesthetic modifications.  
...
# Sort the data by the factor used for fill to obtain desired appearance. This can take some experimentation to achieve optimal results.
...
```{r ggheatmap_prep}
# Determine the order of OTUs based on means within Lung specimens
otu_order <- names(sort(colMeans(otu_lung_df[, stringr::str_detect(colnames(otu_lung_df), "Otu")]), decreasing = TRUE))
...
# Add Genus levels to the OTU Number
levels(heat_df$OTU) <- fct_inorder(paste0(otu_good_taxonomy[levels(heat_df$OTU), ]$Genus, " (", levels(heat_df$OTU), ")"))
```
...
#### 9.1.3.b Base R implementation & dendrogram  
...
bc_lung_spec <- vegan::vegdist(dplyr::select(heat_df, otu_order[1:20]), method = "bray")
...
bc_lung_otu <- vegan::vegdist(t(dplyr::select(heat_df, otu_order[1:20])), method = "bray")
clust_lung_otu <- hclust(bc_lung_otu, method = "average")

# Create the heatmap with dendrograms
heatmap(as.matrix(dplyr::select(heat_df, str_which(colnames(heat_df), "Otu"))), Rowv = as.dendrogram(clust_lung_spec), 
        Colv = as.dendrogram(clust_lung_otu), margins=c(5,0))
```

## 9.2 Relative Abundance of Key Community Members  
### 9.2.1 Random Forests  
#### 9.2.1.a Regression  
With a few minor tweaks to the prior classification forest code for data preparation, retrieval, and plotting, we'll demonstrate how to retrieve feature importances from a collection of random forest regression models.  In this example, we'll use log_IL1a as the response variable.  

```{r many_rf_reg, cache = TRUE}
# For lung specimens, retain only log_IL1a & OTU columns
rf_reg_df <- dplyr::filter(fam_df, tissue_type == "Lung") %>% 
  dplyr::select(log_IL1a, levels(otu_good_taxonomy$Family)) %>% 
  droplevels()

a <- Sys.time()
nforest <- 100
set.seed(rnd_seed) # seed for reproducibility
rf_reg_list <- vector("list", nforest) # Initialize a list to store the randomForests
for (i in seq_along(1:nforest)) { # Loops nforest number of times, creating a model and saving it into the list each time.
    rf_reg_list[[i]] <- randomForest(log_IL1a ~ ., data = rf_reg_df, importance = TRUE) 
}
b <- Sys.time()
b-a

# Extract the feature importances
reg_fi_df <- purrr::map(rf_reg_list, ~{.[["importance"]][, 1]}) %>% # 1 specified the index of the importance column (%IncMSE - regression)
  unlist() %>%
  data.frame(Feature = names(.), X.IncMSE = .)

meas <- colnames(reg_fi_df)[2] # "X.IncMSE"
...
# Retain only the top features
reg_fi_df <- dplyr::filter(reg_fi_df, Feature %in% top_feat$Feature) %>% 
  droplevels() %>%
  rename(IncMSE=X.IncMSE)
reg_fi_df$Feature <- forcats::fct_reorder(reg_fi_df$Feature, reg_fi_df[, 2], .fun = mean, .desc = TRUE)
```

```{r rf_feature_importance_reg, fig.height=10, fig.width=8}
ggplot(reg_fi_df, aes(x = Feature, y = reg_fi_df[, 2])) + 
  geom_boxplot(outlier.alpha = 0.4) + 
  theme_bw() + 
  labs(y = "IncMSE", title = paste0("Feature Importance: log_IL1a ~ Family microbiome"), 
       subtitle = paste0("Model: rf_df \nRuns: ", length(rf_reg_list), ", mtry: ", rf_reg_list[[1]]$mtry)) + 
  coord_flip() + # Flip the axes for improved readability
  scale_x_discrete(limits = rev(levels(reg_fi_df$Feature)))
```
