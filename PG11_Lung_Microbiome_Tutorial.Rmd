...
pca_con_lung_mat <- otu_good[con_lung_df$specimen, ]
otu_con_lung_hel <- decostand(pca_con_lung_mat, "hellinger") # Standardize: use OTU columns only
...
# Keep only the rows of interest for unstandardized data.
pca_lung_mat <- otu_good[otu_lung_df$specimen, ]
otu_lung_hel <- decostand(pca_lung_mat, "hellinger") # Standardize use OTU columns only
...
biplot_results <- quiet_biplot(otu_lung_pca, type = "t", font = 2, font.lab = 2, xlab = "PC1 (14.58% Explained)", ylab = "PC2 (10.68% Explained)",
...
# biplot(prcomp(otu_lung_hel), xlabs=ifelse(str_detect(otu_lung$vendor, "Charles_River"), "CR", "JL")) # arrow.len = 0 will also remove warnings, at the cost of losing the arrowhead
```
...
The following is an example using `vegan::cca()` on the `Cage` variable. For more examples of CCA and other ordination techniques, check out the [Intro to Vegan](https://cran.r-project.org/web/packages/vegan/vignettes/intro-vegan.pdf).  

```{r constrained}
# Run CCA on our hellinger transformed data. We'll need to pass in one or more vectors of metadata corresponding to the constraining variables.
...
ordispider(otu_lung_pca, choices = c(1,3), otu_lung_df$vendor, label = TRUE) # Draw lines from centroids to the points belonging to each group.
legend("topright", levels(otu_lung_df$vendor), pch = 19, col = pcapal, title = "Vendor") # Format and add details to legends
```
